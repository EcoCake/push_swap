...
/*   Updated: 2025/03/06 14:44:57 by amezoe           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (max_of_list(stack) == (*stack)->value
        && min_of_list(stack) == (*stack)->next->next->value)
        {
            swap_a(stack, 0);
            rev_rotate_a(stack, 0);
        }
    else if (max_of_list(stack) == (*stack)->next->value &&
        min_of_list(stack) == (*stack)->value)
        {
            swap_a(stack, 0);
            rotate_a(stack, 0);
        }
...
void less_moves_top_b(t_stack **stack_a, t_stack **stack_b)
{
    ...
    if (least_moves == NULL || least_moves->target == NULL)  // NULL Check
        return;

    if (least_moves->above_medium == 1 && least_moves->target->above_medium == 1)
        rotate_ab_both(stack_a, stack_b, least_moves);
    else if (least_moves->above_medium == 0 && least_moves->target->above_medium == 0)
        rev_rotate_both(stack_a, stack_b, least_moves);

    onto_top_a(stack_a, least_moves);
    onto_top_b(stack_b, least_moves->target);
...
void turks_algo(t_stack **stack_a, t_stack **stack_b)
{
    ...
    while ((*stack_b) != NULL)
    {
        index_def(stack_a);
        update_b(stack_a, stack_b);
        onto_top_a(stack_a, (*stack_b)->target);
